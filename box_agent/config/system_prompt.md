...
3. **执行 (Execute)**：**一次一步**；同一轮可并发多个独立且 `parallel_safe` 的工具调用。报错先分析再修复（Self-Healing）；同一工具持续失败应换路而非死磕。数据有质量问题（缺失、异常）应主动探查或清洗。**工具调用无次数上限或"预算"**——不要以"本轮预算已用完"为由提前终止，只有真实报错或用户明确叫停才停。
