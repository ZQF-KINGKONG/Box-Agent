...
if (process.env.NODE_PATH) {
  process.env.NODE_PATH = `${managedNodeModules}${path.delimiter}${process.env.NODE_PATH}`;
} else {
  process.env.NODE_PATH = managedNodeModules;
}
Module._initPaths();

function usage() {
  console.error(
    "Usage: html_to_pptx.js deck.html output.pptx [--out slides] [--width 1280] [--height 720]"
  );
  process.exit(2);
}
...
async function main() {
  ...
  const selfCheck = await page.evaluate(
    ({ expectedWidth, expectedHeight }) => {
      ...
      return { ok: issues.length === 0, slideCount: slideEls.length, issues, warnings };
    },
    { expectedWidth: opts.width, expectedHeight: opts.height }
  );
  if (!selfCheck.ok) {
    await browser.close();
    console.error("HTML self-check failed before screenshot/export:");
    console.error(JSON.stringify(selfCheck, null, 2));
    process.exit(1);
  }
  if (selfCheck.warnings.length) {
    console.warn("HTML self-check warnings:");
    console.warn(JSON.stringify(selfCheck.warnings, null, 2));
  }
  ...
  for (let i = 0; i < slides.length; i += 1) {
    ...
    titles.push(
      await slide.evaluate((el, idx) => el.getAttribute("data-title") || `Slide ${idx + 1}`, i)
    );
    texts.push(await slide.evaluate(el => (el.innerText || "").replace(/\s+/g, " ").trim()));
  }
  await browser.close();
  ...
  pptx.subject = path.basename(htmlPath);
  ...
  for (let i = 0; i < images.length; i += 1) {
    ...
    slide.addNotes(`Title: ${titles[i]}\n\nText:\n${texts[i]}`);
    slide.addText(texts[i] || titles[i], {
      x: 0,
      y: 0,
      w: 0.01,
      h: 0.01,
